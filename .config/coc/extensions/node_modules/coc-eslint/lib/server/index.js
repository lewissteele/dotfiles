...
function getFilePath(documentOrUri) {
    ...
    let uri = types_1.Is.string(documentOrUri)
        ? vscode_uri_1.default.parse(documentOrUri)
        : vscode_uri_1.default.parse(documentOrUri.uri);
    if (uri.scheme !== 'file') {
        return undefined;
    }
...
function resolveSettings(document) {
    ...
    resultPromise = connection.workspace
        ...
        if (uri.scheme === 'file') {
            ...
            promise = util_1.resolveModule('eslint', directory, settings.resolvedGlobalPackageManagerPath);
        }
        else {
            let localPath = settings.workspaceFolder ? settings.workspaceFolder.uri : undefined;
            promise = util_1.resolveModule('eslint', localPath, settings.resolvedGlobalPackageManagerPath);
        }
